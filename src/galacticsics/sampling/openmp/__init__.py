...
from galacticsics.sampling.openmp.pack import flat_to_particle_dtype, pack_disk_context, pack_halo_context
...
class SamplerBackendStatus:
    """Resolved IC sampler backend for disk/halo components."""
...
def openmp_sampler_status(config: "SampleConfig | None" = None) -> SamplerBackendStatus:
    """Return the disk/halo sampler backend that :func:`sample_disk_python` would use."""
...
def warn_python_sampler_fallback(
    ...
    *,
    ...
    """Emit a visible warning when disk/halo sampling falls back to Python."""
    msg = (
