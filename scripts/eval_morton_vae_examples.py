#!/usr/bin/env python3
"""
Near-production Morton VAE: large-N train (DeepSets + subsampled Chamfer),
converge, sample ~1e5 particles, short OpenMP BH evolve, write face-on PNGs.
"""
...
DT = 0.03
SEED = 0
# Dynamics-based component CE + easy mix prior + virial for evolve fidelity
LAMBDA_CE = 1.0
LAMBDA_MIX = 1.0
LAMBDA_VIRIAL = 2.0
VIRIAL_N_SUB = 256
...
EXAMPLES = [
    ...
    },
    {
        "name": "bar_07b0_late",
        "run": "07b06d08981d",
        "dump": "evolution/particles/step_004000.npz",
        "label": "mild bar / late (A₂≈0.23)",
    },
    {
        "name": "quiet_ic_081e",
        "run": "081ed8af4b2b",
...
def _summarize(name: str, pos: np.ndarray, tid: np.ndarray) -> None:
    r = np.linalg.norm(pos, axis=1)
    counts = {i: int((tid == i).sum()) for i in range(3)}
    print(
        f"  {name}: n={len(pos)}  classes={counts}  "
        f"|x| mean/med/p90={r.mean():.2f}/{np.median(r):.2f}/{np.percentile(r, 90):.2f}",
        flush=True,
    )


def train_vae(manifest: Path) -> tuple[SequenceVAE, list[str]]:
    ckpt = OUT / "sequence_vae_prod.pt"
    if ckpt.is_file():
        ckpt.unlink()
        print(f"removed stale checkpoint {ckpt}", flush=True)
    ...
    print(
        f"train on {len(ds)} snapshots  N={N_TRAIN}  max_epochs={MAX_EPOCHS}  "
        f"patience={PATIENCE}  phase-CE+mixMSE+virial  center+z-rot",
        flush=True,
    )
    print("preloading snapshots into RAM…", flush=True)
    ds.preload()
    ...
    cfg = SequenceVAEConfig(
        ...
        virial_n_sub=VIRIAL_N_SUB,
        lambda_virial_target=1.5,
        map_n_pix=32,
        profile_n_bins=16,
    ...
    print(f"SequenceVAE params={n_params:,}", flush=True)
    ...
    for epoch in range(MAX_EPOCHS):
        ...
        row = {
            "epoch": float(epoch + 1),
            "loss": mean_loss,
            "mse_dx": float(last["mse_dx"].detach()),
            "chamfer": float(last["chamfer"].detach()),
            "ce": float(last["ce"].detach()),
            "ce_acc": float(last["ce_acc"].detach()),
            "mix_mse": float(last["mix_mse"].detach()),
            "nax": float(last["nonaxisym"].detach()),
            "maps": float(last["maps"].detach()),
            "sigma": float(last["sigma"].detach()),
            "virial": float(last["virial"].detach()),
            "ratio_pred": float(last["ratio_pred"].detach()),
            "ratio_data": float(last["ratio_data"].detach()),
            "resid": float(model.resid_scale.detach()),
        }
        history.append(row)
        print(
            f"  epoch {epoch + 1}/{MAX_EPOCHS}  loss={mean_loss:.2f}  "
            f"ch={row['chamfer']:.2f}  ce={row['ce']:.3f}  acc={row['ce_acc']:.3f}  "
            f"mix={row['mix_mse']:.4f}  nax={row['nax']:.2f}  maps={row['maps']:.2f}  "
            f"vir={row['virial']:.3f}  2K/|W|={row['ratio_pred']:.2f}/{row['ratio_data']:.2f}",
            flush=True,
        )
        improved = mean_loss < best_loss * (1.0 - MIN_DELTA)
        ...
        if improved:
            ...
            stall += 1
            if stall >= PATIENCE:
                print(f"  converged @ epoch {epoch + 1}  best_loss={best_loss:.2f}", flush=True)
                break
    ...
    print(f"wrote {ckpt}  best_loss={best_loss:.2f}  epochs={len(history)}", flush=True)
    return model, ds.theta_keys
...
def main() -> int:
    ...
    model, _keys = train_vae(manifest)
    model.eval()
    ...
    for ex in EXAMPLES:
        ...
        if not dump_path.is_file():
            print(f"skip missing {dump_path}", flush=True)
            continue
        print(f"\n=== {ex['name']}: {ex['label']} ===", flush=True)
        ...
        _summarize("data", data["pos"], data["type_id"])
        print(f"  data A2/A0={a2_data:.3f}  t_gyr≈{t_gyr}", flush=True)
        ...
        _summarize("VAE", gen["pos"], gen["type_id"])
        print(f"  generated A2/A0={a2_gen:.3f}  2K/|W|={vr_gen:.3f}", flush=True)
        ...
        _faceon_panel(
            ...
            gen["pos"],
            gen["type_id"],
            f"VAE N={N_GEN // 1000}k  A₂={a2_gen:.2f}  Q={vr_gen:.2f}",
        )
        _faceon_panel(
            axes[2],
            epos,
            sub_tid,
            f"OpenMP BH {EVOLVE_GYR} Gyr  A₂={a2_evo:.2f}  Q={vr_evo:.2f}",
        )
        axes[0].legend(loc="upper right", fontsize=7, markerscale=3)
        fig.suptitle(f"{ex['label']}  ·  run {ex['run']}", y=1.02)
        fig.tight_layout()
